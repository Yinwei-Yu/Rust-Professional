...
pub fn are_anagrams(s1: String, s2: String) -> bool {
    ...
    let s1:String=s1.chars().filter(|ch|ch.is_alphabetic()).collect::<String>().to_lowercase();
    let s2:String=s2.chars().filter(|ch|ch.is_alphabetic()).collect::<String>().to_lowercase();
    for ch in s1.chars() {
        if s2.contains(ch) {
            continue;
        }
